	...
	namespace binsort {
		...
		BinSort::BinSort() {
			...
			if (iss0 >> num) {
				...
				ifs.open(cmd);
				if (!ifs) {
					throw runtime_error("BinSort::BinSort(): Cannot reopen specified file!");
				}
				sort(ifs);
				ifs.close();
			}
		...
		void BinSort::add(size_t added) {
			const long long root = find_parent_node(this->root, data[added].data);
			if (root < 0) {
				return;
			}
			if (data[added].data < data[root].data) {
				const unbalance unb = add_left(root, added);
				if (unb.subtree < 0) {
					return;
				}
				parse_unbalance(unb);
				...
				const unbalance unb = add_right(root, added);
				if (unb.subtree < 0) {
					return;
				}
				parse_unbalance(unb);
		...
		BinSort::unbalance BinSort::add_left(size_t subtree, size_t added) const {
		...
		BinSort::unbalance BinSort::add_right(size_t subtree, size_t added) const {
		...
		BinSort::unbalance BinSort::find_smallest_unbalance(long long start, bool current) const {
		...
		void BinSort::set_depth(size_t node) const {
