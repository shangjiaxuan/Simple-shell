...
namespace UJr2_funcs {
	namespace book {
		...
		int list::add(const std::string& name) {
			return add(-1, name);
		}
		...
		void list::add_head(int index, const std::string& name, bool fixed) {
			head = new volume;
		...
		void list::change_head(int index, const std::string& name, bool fixed) {
			volume* temp = head;
			head = new volume;
			head->name = name;
			head->index_number = index;
		...
		void list::add(volume* previous, int index, const std::string& name, bool index_type) {
		...
		*/
		...
		void list::save(std::ofstream& ofs) const {
			print(ofs);
			/*	if (head) {
					volume* current = head;
					while (true) {
