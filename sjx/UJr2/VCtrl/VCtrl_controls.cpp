...
namespace UJr2_funcs {
	namespace vehicle_control {
		...
		bool controls::file_add_violation(const std::string& cmd) {
			...
			while(!ifs.eof()) {
				getline(ifs, entry);
				if(manual_add_violation(entry)) {} else {
		...
		std::vector<controls::found_violation> controls::
		core_find_violation_loaction(const std::string& location) const {
			vector<found_violation> found;
			for(int i = 0; i < car_num; i++) {
				if(!all_cars[i].violations) {
					continue;
