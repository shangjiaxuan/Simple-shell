	...
	namespace crossing {
		namespace crossing {
			...
			*/
			...
			short relations::left_dist(short s1, short s2) {
				//两个路口，s1来看s2在它左面几个
			...
			short relations::right_dist(short s1, short s2) {
				//s1来看s2在它右面几个
			...
			char relations::rela(Roads::route rt, short pos) {
				//看一个路口在一条路左面还是右面。a为左，b为右，c为与到达点重合，d为与出发点重合
			...
			bool relations::compatible(Roads::route& rt1, Roads::route& rt2) {
				//注意靠右行驶的问题
				if(rt1.from == rt2.from && rt1.to == rt2.to) {
					//在创建表的时候有这一项。。。
				...
				if(rela(rt1, rt2.from) == rela(rt1, rt2.to)) {
					//先考虑一般情况，并且认为已经处理好了掉头
