...
class UI {
	...
	private:
		...
		static nstring parse_input();			//for pasring a single argument
		static std::vector<nstring> Get_input();//for making vector of parsed arguments
												//and then pass on to the command parser
		static bool change(const char& a);		//certain characters are supported on console
